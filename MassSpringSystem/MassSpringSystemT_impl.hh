...
namespace AOPT {
    ...
    void MassSpringSystemT<MassSpringProblem>::add_area_constraints() {
        ...
        double epsilon = 1e-10;  // A small value for numerical stability

        // Iterate over all grid squares (which are composed of two triangles)
        for (int j = 0; j < n_grid_y_; ++j) {
            for (int i = 0; i < n_grid_x_; ++i) {
                // Define the 3 nodes for the first triangle (top-left triangle)
                int idx0 = get_grid_index(i, j);
                int idx1 = get_grid_index(i + 1, j);
                int idx2 = get_grid_index(i, j + 1);

                // Define the 3 nodes for the second triangle (bottom-right triangle)
                int idx3 = get_grid_index(i + 1, j + 1);
                int idx4 = idx2;
                int idx5 = idx1;

                // Ensure both triangles have consistent orientation (counter-clockwise)
                // Triangle 1: (idx0, idx1, idx2)
                double x0 = sg_.point(idx0)[0], y0 = sg_.point(idx0)[1];
                double x1 = sg_.point(idx1)[0], y1 = sg_.point(idx1)[1];
                double x2 = sg_.point(idx2)[0], y2 = sg_.point(idx2)[1];

                double area1 = (x1 - x0) * (y2 - y0) - (x2 - x0) * (y1 - y0);  // Signed area

                if (area1 < 0.0) {
                    // Swap idx1 and idx2 to ensure counter-clockwise orientation
                    std::swap(idx1, idx2);
                }

                // Triangle 2: (idx3, idx4, idx5)
                x0 = sg_.point(idx3)[0], y0 = sg_.point(idx3)[1];
                x1 = sg_.point(idx4)[0], y1 = sg_.point(idx4)[1];
                x2 = sg_.point(idx5)[0], y2 = sg_.point(idx5)[1];

                double area2 = (x1 - x0) * (y2 - y0) - (x2 - x0) * (y1 - y0);  // Signed area

                if (area2 < 0.0) {
                    // Swap idx4 and idx5 to ensure counter-clockwise orientation
                    std::swap(idx4, idx5);
                }

                // Add constraints for both triangles
                area_constraints_.emplace_back(n_unknowns_, idx0, idx1, idx2, epsilon);  // Triangle 1
                area_constraints_.emplace_back(n_unknowns_, idx3, idx4, idx5, epsilon);  // Triangle 2
            }
        }
